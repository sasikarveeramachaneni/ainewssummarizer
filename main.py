...
fetcher_agent = Agent(role="News Fetcher", backstory="Scrapes and extracts article text and heading.", goal="Fetch article heading and text.")
...
def categorize_news(article_text):
    model = genai.GenerativeModel("gemini-1.5-flash-latest")
    trimmed_text = article_text[:3000]
    prompt = f"Categorize this news article: '{trimmed_text}' into a category like Technology, Politics, Sports, etc."
    print("Fetching Category...")
    return generate_with_retry(model, prompt, role="Categorizer")

categorizer_agent = Agent(role="Categorizer", backstory="Classifies news articles.", goal="Categorize articles.")

def summarize_article(article_title, article_text):
    model = genai.GenerativeModel("gemini-1.5-flash-latest")
    trimmed_text = article_text[:7000]
    prompt = (f"Here is a news article titled '{article_title}'.\n\n"
              f"Summarize this article in a detailed way with a minimum of 200 words:\n\n{trimmed_text}")
    print("Generating Summary...")
    return generate_with_retry(model, prompt, role="Summarizer")

summarizer_agent = Agent(role="Summarizer", backstory="Generates detailed article summaries of at least 200 words.", goal="Summarize articles in a detailed way.")
